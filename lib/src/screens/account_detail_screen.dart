  ...
  Future<void> _showAdminPanel() async {
    ...
    if (!data.adminLoggedIn || !data.adminFullAccess) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Bu bölüm gizli: önce kimo@ ile yönetici şifresiyle giriş yapın',
          ),
        ),
      );
      return;
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) {
          ...
          return AlertDialog(
            ...
            title: const Text('Yönetim paneli — Takımlar ve oyuncular'),
            content: SizedBox(
              width: 640,
              height: 500,
              child: Column(
                      ...
                      children: [
                        for (final player in players)
                          ListTile(
                            ...
                            subtitle: Text(
                              'Şut ${player.shootingRating.toStringAsFixed(0)}'
                              ' | Pas ${player.passingRating.toStringAsFixed(0)}'
                              ' | Hız ${player.speedRating.toStringAsFixed(0)}'
    ...
    await showDialog<void>(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          backgroundColor: const Color(0xff102019),
          title: Text('${player.name} düzenle'),
          content: SizedBox(
            width: 520,
            child: SingleChildScrollView(
              child: Column(
                children: [
                  _adminEditorSlider('Genel oyun', player.overallRating, (v) {
                    setDialogState(() => player.overallRating = v);
                  }),
                  _adminEditorSlider('Şut', player.shootingRating, (v) {
                  ...
                  _adminEditorSlider('Şut gücü', player.shotPowerRating, (v) {
                    setDialogState(() => player.shotPowerRating = v);
                  }),
                  _adminEditorSlider('Uzaktan şut', player.longShotsRating, (v) {
                    setDialogState(() => player.longShotsRating = v);
                  }),
                  _adminEditorSlider('Falso', player.curveRating, (v) {
                    setDialogState(() => player.curveRating = v);
                  }),
                  _adminEditorSlider('Soğukkanlılık', player.composureRating, (v) {
                  ...
                  _adminEditorSlider('Pas / Top saklama', player.passingRating, (v) {
                    setDialogState(() => player.passingRating = v);
                  }),
                  _adminEditorSlider('Hız', player.speedRating, (v) {
                    setDialogState(() => player.speedRating = v);
                  }),
                  _adminEditorSlider('Dayanıklılık', player.staminaRating, (v) {
                    setDialogState(() => player.staminaRating = v);
                  }),
                  _adminEditorSlider('Kalecilik', player.goalkeepingRating, (v) {
                    setDialogState(() => player.goalkeepingRating = v);
                  }),
                  if (player.isGoalkeeper) ...[
                    _adminEditorSlider('Reaksiyon', player.goalkeeperReactionRating, (v) {
                    ...
                    _adminEditorSlider('Atlayış', player.goalkeeperDivingRating, (v) {
                      setDialogState(() => player.goalkeeperDivingRating = v);
                    }),
                    _adminEditorSlider('Kontrol', player.goalkeeperHandlingRating, (v) {
                      setDialogState(() => player.goalkeeperHandlingRating = v);
                    }),
                    _adminEditorSlider('Tutuş', player.goalkeeperCatchingRating, (v) {
                      setDialogState(() => player.goalkeeperCatchingRating = v);
                    }),
                    _adminEditorSlider('Sıçrama', player.goalkeeperJumpingRating, (v) {
                    ...
                    _adminEditorSlider('Yüksek top', player.goalkeeperHighBallsRating, (v) {
                      setDialogState(() => player.goalkeeperHighBallsRating = v);
                    }),
                    _adminEditorSlider('Erişim', player.goalkeeperReachRating, (v) {
                      setDialogState(() => player.goalkeeperReachRating = v);
                    }),
                    _adminEditorSlider('Tahmin', player.goalkeeperAnticipationRating, (v) {
                      setDialogState(() => player.goalkeeperAnticipationRating = v);
                    }),
                    _adminEditorSlider('Uzaklaştırma', player.goalkeeperParryingRating, (v) {
                      setDialogState(() => player.goalkeeperParryingRating = v);
                    }),
                    _adminEditorSlider('Dağıtım', player.goalkeeperDistributionRating, (v) {
                  ...
                  _adminEditorSlider('Zeka', player.zekaGucu, (v) {
                    setDialogState(() => player.zekaGucu = v);
                  }),
                  ListTile(
                    contentPadding: EdgeInsets.zero,
                    title: const Text('Maç cezası'),
                    subtitle: Text(
                      'Kartlar: ${player.yellowCards} sarı, ${player.redCards} kırmızı',
                    ),
