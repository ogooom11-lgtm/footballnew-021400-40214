...
class _GameScreenState extends State<GameScreen>
  ...
  void _onTick(Duration elapsed) {
    ...
    // VAR goal detail playback: loop the captured approach animation
    // (مطلب: انميشن شغال يعيد ما حدث بوضوح).
    if (!_engine.varReviewActive && _varDetailOpen) {
      _varDetailOpen = false;
    }
    if (_varDetailOpen && _varDetailPlaying &&
        _engine.goalReplayFrames.length > 1) {
      _varDetailTime += dt * _varDetailSpeed;
      if (_varDetailTime >= _varDetailDuration) {
        _varDetailTime -= _varDetailDuration;
      }
    }
    if (mounted) {
      setState(() {});
  ...
  // ---------- VAR animated goal detail (مطلب: انميشن يعيد الهدف من
  // فوق ومن الأمام بدل صورة ثابتة) ----------
  bool _varDetailOpen = false;
  double _varDetailTime = 0;
  double _varDetailSpeed = 0.5;
  bool _varDetailPlaying = true;

  /// Total replay duration of the captured goal approach.
  double get _varDetailDuration =>
      math.max(0.2, (_engine.goalReplayFrames.length - 1) * 0.08);
  ...
  void _onKey(KeyEvent event) {
    ...
    if (event is KeyDownEvent || event is KeyRepeatEvent) {
      ...
      if (key == LogicalKeyboardKey.escape) {
        if (_varDetailOpen) {
          // Close the animated VAR goal detail overlay first
          // (مطلب: فتح واغلاق شاشة المراقبة).
          setState(() => _varDetailOpen = false);
        } else if (_subTeam != null) {
        ...
        body: SizedBox.expand(
          child: Stack(
            children: [
              ...
              // Animated VAR goal detail overlay — top + front cameras
              // replaying the goal (مطلب: انميشن من فوق ومن الأمام).
              if (_varDetailOpen &&
                  _engine.varReviewActive &&
                  _engine.varReviewCategory == 'goal' &&
                  _engine.goalReplayFrames.isNotEmpty &&
                  !_engine.replayMode &&
                  !_engine.finished)
                _varGoalDetailOverlay(),
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_engine.varReviewActive) ...[
                    ...
                    // Animated goal detail cameras (مطلب: زر تفصيل/مراقبة
                    // على كل هدف يعيد اللقطة بالانميشن من فوق ومن الأمام).
                    if (_engine.varReviewCategory == 'goal' &&
                        _engine.goalReplayFrames.length > 1) ...[
                      FilledButton.icon(
                        onPressed: () {
                          setState(() {
                            _varDetailOpen = true;
                            _varDetailTime = 0;
                            _varDetailPlaying = true;
                          });
                        },
                        style: FilledButton.styleFrom(
                          backgroundColor: const Color(0xffb388ff),
                          foregroundColor: const Color(0xff160b2e),
                          padding: const EdgeInsets.symmetric(
                            horizontal: 12,
                            vertical: 8,
                          ),
                        ),
                        icon: const Icon(Icons.slow_motion_video, size: 17),
                        label: const Text(
                          'Detayli izle — kamerayi ac',
                          style: TextStyle(
                            fontSize: 12,
                            fontWeight: FontWeight.w900,
                          ),
  ...
  // =====================================================================
  // =====================================================================
  // VAR animated goal detail (مطلب: زر تفصيل/مراقبة على كل هدف يفتح
  // انميشن يعيد اللقطة من فوق ومن الأمام بوضوح بدل صورة ثابتة، مع
  // الحارس بكل حالاته: واقف، قافز، طائر، على الأرض).
  // =====================================================================

  /// Samples the captured goal replay at the current playback time and
  /// returns ball + keeper positions (with the keeper's previous frame so
  /// painters can derive dive direction and speed).
  ({
    double bx,
    double by,
    double bh,
    ...
    bool crossed,
      ...
      _sampleGoalReplay() {
    final frames = _engine.goalReplayFrames;
    final keeperId = _engine.goalKeeperId;
    final view = _engine.goalVarView;
    final centre = GameConstants.virtualHeight / 2;
    final t = _varDetailTime.clamp(0.0, _varDetailDuration);
    final pos = t / 0.08;
    final i0 = pos.floor().clamp(0, frames.length - 1);
    final i1 = (i0 + 1).clamp(0, frames.length - 1);
    final f = (pos - i0).clamp(0.0, 1.0);
    double lerp(double a, double b) => a + (b - a) * f;

    (double, double) keeperAt(ReplayFrame frame) {
      for (final p in frame.players) {
        if (p.id == keeperId) {
          return (p.x, p.y);
        }
      }
      final side = view?.concedingSide ?? TeamSide.left;
      return (
        side == TeamSide.left
    ...
    final fa = frames[i0];
    final fb = frames[i1];
    final (kax, kay) = keeperAt(fa);
    final (kbx, kby) = keeperAt(fb);
    final (kpx2, kpy2) = keeperAt(frames[(i0 - 1).clamp(0, frames.length - 1)]);
    return (
      bx: lerp(fa.ballX, fb.ballX),
      by: lerp(fa.ballY, fb.ballY),
      bh: lerp(fa.ballHeight, fb.ballHeight),
      kx: lerp(kax, kbx),
      ky: lerp(kay, kby),
      kpx: kpx2,
      kpy: kpy2,
      index: i0,
      crossed: i1 >= frames.length - 1 && f > 0.55,
    );
  }

  /// Sample for the trail at an earlier time offset (seconds back).
  ({double bx, double by, double bh})? _sampleTrailPoint(double back) {
    final frames = _engine.goalReplayFrames;
    if (frames.length < 2) return null;
    final t = (_varDetailTime - back).clamp(0.0, _varDetailDuration);
    final pos = t / 0.08;
    final i0 = pos.floor().clamp(0, frames.length - 1);
    final i1 = (i0 + 1).clamp(0, frames.length - 1);
    final f = (pos - i0).clamp(0.0, 1.0);
    ...
    final fb = frames[i1];
    return (
      bx: fa.ballX + (fb.ballX - fa.ballX) * f,
      by: fa.ballY + (fb.ballY - fa.ballY) * f,
      bh: fa.ballHeight + (fb.ballHeight - fa.ballHeight) * f,
    );
  }

  Widget _varGoalDetailOverlay() {
    final view = _engine.goalVarView;
    const accent = Color(0xffb388ff);
    final sample = _sampleGoalReplay();
    final concedingSide = view?.concedingSide ?? TeamSide.left;
    final concedingTeam = _engine.teamBySide(concedingSide);
    final keeperJersey = concedingTeam.goalkeeperKit?.goalkeeperShirtColor ??
        const Color(0xffff8a3d);
    final trail = <({double bx, double by, double bh})>[];
    for (var s = 0.12; s <= 0.97; s += 0.12) {
      final p = _sampleTrailPoint(s);
      if (p != null) trail.add(p);
    }
    final progress = _varDetailDuration == 0
        ? 1.0
        : (_varDetailTime / _varDetailDuration).clamp(0.0, 1.0);
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              gradient: const LinearGradient(
                ...
                colors: [Color(0xff151027), Color(0xff0a0f1e)],
              ),
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    const Text(
                      'GOL DETAY KAMERASI — animasyonlu inceleme',
                      style: TextStyle(
                        color: Colors.white,
                        fontWeight: FontWeight.w900,
                        fontSize: 15,
                ...
                Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // FRONT camera (مطلب: المرمى من الأمام).
                    Expanded(
                      flex: 11,
                      child: Column(
                        ...
                        children: [
                          _cameraLabel(
                            'ONDEN GORUNUM — ${concedingTeam.name} kalesi',
                            Icons.videocam,
                          ),
                          const SizedBox(height: 5),
                          ClipRRect(
                            ...
                            child: SizedBox(
                              height: 330,
                              child: CustomPaint(
                                painter: _VarFrontPainter(
                                  ballPitchY: sample.by,
                                  ballHeight: sample.bh,
                                  keeperPitchY: sample.ky,
                                  keeperPrevPitchY: sample.kpy,
                                  concedingSide: concedingSide,
                                  keeperJersey: keeperJersey,
                                  trail: [
                    ...
                    Expanded(
                      ...
                      child: Column(
                        ...
                        children: [
                          _cameraLabel(
                            'USTTEN GORUNUM — ${concedingTeam.name} kalesi',
                            Icons.map_outlined,
                          ),
                          const SizedBox(height: 5),
                          ClipRRect(
                ...
                Row(
                  children: [
                    IconButton(
                      tooltip:
                          _varDetailPlaying ? 'Duraklat' : 'Oynat',
                      onPressed: () =>
                          setState(() => _varDetailPlaying = !_varDetailPlaying),
                      icon: Icon(
                        _varDetailPlaying
                        ...
                        child: ChoiceChip(
                          label: Text(
                            ...
                            style: TextStyle(
                              fontSize: 11,
                              fontWeight: FontWeight.w900,
                              color: _varDetailSpeed == speed
                                  ? const Color(0xff160b2e)
                                  : Colors.white60,
                            ),
                          ),
                          selected: _varDetailSpeed == speed,
                          selectedColor: accent,
                          backgroundColor:
                              Colors.white.withValues(alpha: 0.08),
                          onSelected: (_) =>
                              setState(() => _varDetailSpeed = speed),
                        ),
                      ),
                    const Spacer(),
                    const Icon(
                ...
                const SizedBox(height: 10),
                // ---------------- VAR decision (مباشرة من الكاميرا) ----------------
                Row(
                  children: [
                    const Text(
                      'VAR karari:',
  ...
  Widget _cameraLabel(String text, IconData icon) {
    return Row(
      children: [
...
/// Animated FRONT camera of the conceding goal for the VAR detail
/// overlay: ball trajectory with height + a fully drawn goalkeeper in his
/// real state — standing, set, diving or lying on the ground
/// (مطلب: الحارس من الأمام بكل حالاته، واضح وجميل).
class _VarFrontPainter extends CustomPainter {
  _VarFrontPainter({
    required this.ballPitchY,
    required this.ballHeight,
    required this.keeperPitchY,
    required this.keeperPrevPitchY,
    required this.concedingSide,
    required this.keeperJersey,
    required this.trail,
    required this.crossed,
    required this.phase,
  });

  final double ballPitchY;
  final double ballHeight;
  final double keeperPitchY;
  final double keeperPrevPitchY;
  final TeamSide concedingSide;
  final Color keeperJersey;
  final List<({double pitchY, double height})> trail;
  final bool crossed;
  final double phase;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final bg = Paint()
      ..shader = const LinearGradient(
        ...
        colors: [Color(0xff0b1322), Color(0xff13223f)],
      ).createShader(Offset.zero & size);
    canvas.drawRect(Offset.zero & size, bg);

    // Stand lights.
    final crowd = Paint()..color = Colors.white.withValues(alpha: 0.05);
    for (var y = size.height * 0.03;
        y < size.height * 0.30;
        y += size.height * 0.034) {
      canvas.drawRect(
        Rect.fromLTWH(0, y, size.width, size.height * 0.013),
        crowd,
      );
    }

    final groundY = size.height * 0.87;
    final grass = Paint()
      ..shader = const LinearGradient(
        begin: Alignment.topCenter,
        end: Alignment.bottomCenter,
        colors: [Color(0xff1e6340), Color(0xff123c27)],
      ).createShader(
    ...
    // ---------- goal ----------
    final goalL = size.width * 0.20;
    final goalR = size.width * 0.80;
    ...
    final net = Paint()
      ...
      ..strokeWidth = 1;
    for (var x = goalL; x <= goalR; x += 12) {
      canvas.drawLine(Offset(x, barY), Offset(x, groundY), net);
    }
    for (var y = barY; y <= groundY; y += 12) {
    ...
    // ---------- helpers: pitch -> screen ----------
    double sx(double pitchY) =>
        goalL +
        ((pitchY - (centre - mouthHalf)) /
                GameConstants.goalPixelHeight) *
            (goalR - goalL);
    double sy(double heightMeters) =>
        groundY -
        (heightMeters / GameConstants.crossbarMaxMeters).clamp(0.0, 1.0) *
            goalH;

    // ---------- keeper state ----------
    final latSpeed = (keeperPitchY - keeperPrevPitchY) / 0.08;
    final offset = ((keeperPitchY - centre) / mouthHalf).clamp(-1.15, 1.15);
    // 0 standing, 1 set, 2 diving, 3 lying (مطلب: كل حالات الحارس).
    int pose;
    if (crossed && offset.abs() > 0.18) {
      pose = 3;
    } else if (latSpeed.abs() > 150) {
      pose = 2;
    } else if (latSpeed.abs() > 70 || offset.abs() > 0.55) {
      pose = 1;
    } else {
      pose = 0;
    }
    final diveDir = latSpeed > 0 ? 1 : -1;

    // Keeper screen position along the line.
    final keeperScreenX = (goalL + (goalR - goalL) * (0.5 + offset * 0.46))
        .clamp(goalL + 16, goalR - 16);

    // Shadow.
    final shadow = Paint()..color = Colors.black.withValues(alpha: 0.35);
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(keeperScreenX, groundY + 2),
        width: 64,
        height: 12,
      ),
      shadow,
    ...
    // ---------- trail ----------
    final trailBall = Paint()..color = Colors.white.withValues(alpha: 0.25);
    for (var i = 0; i < trail.length; i++) {
      final p = trail[i];
      final alpha = 0.10 + 0.22 * (i / trail.length);
      trailBall.color = Colors.white.withValues(alpha: alpha);
      canvas.drawCircle(
        Offset(sx(p.pitchY), sy(p.height)),
        3.4 + 2.0 * (i / trail.length),
        trailBall,
      );
    }

    // ---------- ball ----------
    final ballX = sx(ballPitchY).clamp(goalL - 40, goalR + 40);
    final ballY = sy(ballHeight).clamp(barY - 30, groundY - 4);
    final ballR = crossed ? 9.5 : 8.5;
    if (crossed) {
      final glow = Paint()
        ..shader = RadialGradient(
          colors: [
            const Color(0xff2ee59d).withValues(alpha: 0.6),
            const Color(0xff2ee59d).withValues(alpha: 0.0),
          ],
        ).createShader(
          Rect.fromCircle(center: Offset(ballX, ballY), radius: ballR * 5),
        );
      canvas.drawCircle(Offset(ballX, ballY), ballR * 5, glow);
    }
    ...
    // Pentagon hint on the ball.
    canvas.drawCircle(
      Offset(ballX - ballR * 0.3, ballY - ballR * 0.2),
      ballR * 0.28,
      Paint()..color = const Color(0xff25303a),
    );
    ...
    if (crossed) {
      ...
      tp.paint(
        canvas,
        Offset(size.width / 2 - tp.width / 2, size.height * 0.06),
      );
    }
  }

  /// Draws a full articulated goalkeeper: head, jersey torso, shorts,
  /// two-segment legs and arms with gloves — posed standing, set,
  /// diving (airborne) or lying flat on the ground.
  void _drawKeeper(
    Canvas canvas, {
    ...
    final skinPaint = Paint()
      ..color = const Color(0xffe8b98d)
      ..strokeWidth = h * 0.055
      ..strokeCap = StrokeCap.round;
    final glovePaint = Paint()..color = const Color(0xfff4f6fb);
    final hairPaint = Paint()..color = const Color(0xff2a2118);

    // Body angle from vertical: 0 upright, ~1.05 airborne dive, ~1.5 lying.
    ...
    // Hip height above the ground by pose.
    final hipLift = switch (pose) {
      3 => h * 0.14,
      2 => h * 0.52,
      1 => h * 0.36,
      _ => h * 0.44,
    };
    final hip = Offset(feetX, groundY - hipLift);

    canvas.save();
    canvas.translate(hip.dx, hip.dy);
    canvas.rotate(angle);

    // Local skeleton (upright, hip at origin, -y is up).
    final shoulder = Offset(0, -h * 0.34);
    final headC = Offset(0, -h * 0.47);
    final headR = h * 0.085;

    // ---- legs ----
    if (pose == 3) {
      // Lying flat: both legs trail along the ground behind the torso.
      canvas.drawLine(Offset.zero, Offset(-dir * h * 0.30, h * 0.10), shortsPaint);
      canvas.drawLine(
        Offset(-dir * h * 0.30, h * 0.10),
        Offset(-dir * h * 0.52, h * 0.13),
        skinPaint,
      );
      canvas.drawLine(Offset.zero, Offset(-dir * h * 0.26, h * 0.16), shortsPaint);
      canvas.drawLine(
        Offset(-dir * h * 0.26, h * 0.16),
        Offset(-dir * h * 0.48, h * 0.20),
        skinPaint,
      );
    } else if (pose == 2) {
      // Airborne dive: legs stream behind, slightly split.
      canvas.drawLine(Offset.zero, Offset(-dir * h * 0.20, h * 0.16), shortsPaint);
      canvas.drawLine(
        Offset(-dir * h * 0.20, h * 0.16),
        Offset(-dir * h * 0.38, h * 0.30),
        skinPaint,
      );
      canvas.drawLine(Offset.zero, Offset(-dir * h * 0.14, h * 0.22), shortsPaint);
      canvas.drawLine(
        Offset(-dir * h * 0.14, h * 0.22),
        Offset(-dir * h * 0.30, h * 0.40),
        skinPaint,
      );
    } else {
      // Standing / set: legs down, knees slightly bent when set.
      final kneeBend = pose == 1 ? h * 0.10 : h * 0.20;
      final footSpread = h * 0.10;
      canvas.drawLine(Offset.zero, Offset(-footSpread, kneeBend), shortsPaint);
      canvas.drawLine(
        Offset(-footSpread, kneeBend),
        Offset(-footSpread * 1.2, h * 0.44),
        skinPaint,
      );
      canvas.drawLine(Offset.zero, Offset(footSpread, kneeBend), shortsPaint);
      canvas.drawLine(
    ...
    final torso = Paint()
      ...
      ..strokeWidth = h * 0.16
      ..strokeCap = StrokeCap.round;
    canvas.drawLine(Offset.zero, shoulder, torso);

    // ---- arms + gloves ----
    Offset handL;
    Offset handR;
    if (pose == 2 || pose == 3) {
      // Full stretch along the dive — reaching for the ball.
      handL = Offset(dir * h * 0.06, -h * 0.66);
      handR = Offset(-dir * h * 0.04, -h * 0.60);
    } else if (pose == 1) {
      // Set: arms low and wide in front.
      handL = Offset(-h * 0.24, -h * 0.10);
      handR = Offset(h * 0.24, -h * 0.10);
    } else {
      // Standing: relaxed at the sides, tiny idle sway.
      final sway = math.sin(phase * 2.2) * h * 0.02;
      handL = Offset(-h * 0.17, -h * 0.06 + sway);
      handR = Offset(h * 0.17, -h * 0.06 - sway);
    }
    final elbowL = shoulder + (handL - shoulder) * 0.5 + Offset(0, h * 0.03);
    final elbowR = shoulder + (handR - shoulder) * 0.5 + Offset(0, h * 0.03);
    canvas.drawLine(shoulder, elbowL, jerseyPaint);
    canvas.drawLine(elbowL, handL, skinPaint);
    canvas.drawLine(shoulder, elbowR, jerseyPaint);
    canvas.drawLine(elbowR, handR, skinPaint);
    canvas.drawCircle(handL, h * 0.05, glovePaint);
    canvas.drawCircle(handR, h * 0.05, glovePaint);

    // ---- head ----
    canvas.drawCircle(headC, headR, skinPaint..strokeWidth = 0);
    canvas.drawArc(
      Rect.fromCircle(center: headC, radius: headR),
      math.pi,
      math.pi,
      false,
      hairPaint..strokeWidth = headR * 0.55,
    );
  ...
  @override
  bool shouldRepaint(_VarFrontPainter oldDelegate) => true;
}

/// Animated TOP camera around the conceding goal for the VAR detail
/// overlay: pitch from above, goal frame, keeper and ball trajectory
/// (مطلب: المرمى من فوق وكيف دخلت الكرة).
class _VarTopPainter extends CustomPainter {
  _VarTopPainter({
    required this.ballX,
    required this.ballY,
  ...
  final Color keeperJersey;
  final TeamSide concedingSide;
  final List<({double pitchX, double pitchY})> trail;
  final bool crossed;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final grass = Paint()
      ..shader = const LinearGradient(
        ...
        colors: [Color(0xff1d5c3c), Color(0xff174a30)],
      ).createShader(Offset.zero & size);
    ...
    for (var y = 0.0; y < size.height; y += size.height / 7) {
      canvas.drawRect(
        Rect.fromLTWH(0, y, size.width, size.height / 14),
        stripe,
      );
    }
    ...
    final centre = GameConstants.virtualHeight / 2;
    final mouthHalf = GameConstants.goalPixelHeight / 2;
    final left = concedingSide == TeamSide.left;
    final line = left ? GameConstants.leftBound : GameConstants.rightBound;

    // World -> screen mapping: the area around the conceding goal.
    const worldDepth = 300.0; // px of pitch shown behind the goal line
    const worldWide = 400.0; // px across
    final pad = 18.0;
    double sx(double wx) {
      final d = left ? wx - (line - 70) : (line + 70) - wx;
      return pad + (d / worldDepth) * (size.width - pad * 2);
    }

    double sy(double wy) =>
        size.height / 2 + (wy - centre) * ((size.height - pad * 2) / worldWide);
    ...
    canvas.drawLine(
      Offset(sx(line), 0),
      Offset(sx(line), size.height),
      white,
    );
    // Penalty box + six-yard box + spot + arc.
    final boxW = 240.0;
    final boxD = 130.0;
    canvas.drawRect(
      Rect.fromPoints(
        Offset(sx(line), sy(centre - boxW / 2)),
        Offset(sx(left ? line + boxD : line - boxD), sy(centre + boxW / 2)),
      ),
      white,
    );
    final sixW = 140.0;
    ...
    canvas.drawRect(
      Rect.fromPoints(
        Offset(sx(line), sy(centre - sixW / 2)),
        Offset(sx(left ? line + sixD : line - sixD), sy(centre + sixW / 2)),
      ),
      white,
    );
    final spotX = sx(left ? line + 95 : line - 95);
    canvas.drawCircle(Offset(spotX, sy(centre)), 3, Paint()..color = Colors.white);
    // D-arc.
    canvas.drawArc(
      Rect.fromCircle(
        center: Offset(spotX, sy(centre)),
        radius: 46,
      ),
    ...
    // Goal frame from above: posts + net behind the line.
    final postPaint = Paint()..color = const Color(0xfff2f4f8);
    final postTop = sy(centre - mouthHalf);
    final postBot = sy(centre + mouthHalf);
    final netDepth = sx(left ? line - GameConstants.goalDepth : line + GameConstants.goalDepth);
    final lineX = sx(line);
    final netRect = Rect.fromLTRB(
      math.min(lineX, netDepth),
      postTop,
      math.max(lineX, netDepth),
      postBot,
    );
    canvas.drawRect(
      netRect,
    ...
    final netLine = Paint()
      ...
      ..strokeWidth = 1;
    const netN = 6;
    for (var i = 0; i <= netN; i++) {
      final yy = postTop + (postBot - postTop) * (i / netN);
      canvas.drawLine(Offset(netRect.left, yy), Offset(netRect.right, yy), netLine);
    }
    for (var i = 0; i <= 3; i++) {
      final xx = netRect.left + netRect.width * (i / 3);
      canvas.drawLine(Offset(xx, postTop), Offset(xx, postBot), netLine);
    }
    canvas.drawCircle(Offset(lineX, postTop), 4, postPaint);
    canvas.drawCircle(Offset(lineX, postBot), 4, postPaint);

    // ---------- ball trail ----------
    for (var i = 0; i < trail.length; i++) {
      final p = trail[i];
      final alpha = 0.12 + 0.30 * (i / math.max(1, trail.length - 1));
      canvas.drawCircle(
        Offset(sx(p.pitchX), sy(p.pitchY)),
        3 + 2.4 * (i / math.max(1, trail.length - 1)),
        Paint()..color = Colors.white.withValues(alpha: alpha),
      );
    }

    // ---------- keeper ----------
    final kPos = Offset(sx(keeperX), sy(keeperY));
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(kPos.dx, kPos.dy + 3),
        width: 22,
        height: 8,
      ),
      Paint()..color = Colors.black.withValues(alpha: 0.3),
    );
    canvas.drawCircle(kPos, 8, Paint()..color = keeperJersey);
    canvas.drawCircle(
      kPos,
      8,
      Paint()
    ...
    // ---------- ball ----------
    final bPos = Offset(sx(ballX), sy(ballY));
    if (crossed) {
      final glow = Paint()
        ..shader = RadialGradient(
          colors: [
        ...
        ).createShader(Rect.fromCircle(center: bPos, radius: 34));
      canvas.drawCircle(bPos, 34, glow);
    }
    canvas.drawCircle(bPos, 6, Paint()..color = Colors.white);
    canvas.drawCircle(
      bPos,
      6,
      Paint()
  ...
  @override
  bool shouldRepaint(_VarTopPainter oldDelegate) => true;
}
